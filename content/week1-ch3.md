...
可以在两者之间做平滑折中：让相邻下一步的修正保留较大权重，距离越远，权重越小。
用 \(\lambda\in[0,1]\) 表示每向前传播一步时额外保留的比例：
...
这套“把不同距离的后续修正按衰减权重加回来”的方法，叫
**generalized advantage estimation（广义优势估计）**，简称 **GAE**。
