...
到这里，critic 已经为每一步给出局部预测修正，但策略更新还缺少一件东西：

> 第 \(t\) 个 token 最终应该使用哪个数作为自己的更新权重？

如果直接把当前 TD 当作更新权重，那么上一节的失败回答会得到：
...
前两个片段会被正向强化，终局失败只惩罚最后一个片段。原因很明确：
\(\delta_t\) 只回答“生成这一步后，critic 的预期立刻改变了多少”，它还没有吸收更晚出现的证据。

以第一个片段为例，可以决定向未来看多远：

| 使用的证据 | 第一个片段得到的数 | 含义 |
|---|---:|---|
| 只看当前一步 | \(0.20\) | 完全相信 critic 当时的局部判断 |
| 再看下一步 | \(0.20+0.30=0.50\) | 加入一个后续修正 |
| 一直看到终局 | \(0.20+0.30-0.80=-0.30\) | 吸收整条失败结果 |
...
- 不能只看当前一步，否则太相信尚不准确的 critic；
- 也不必让遥远终局以完整强度影响所有早期 token，否则长回答之间的随机波动会全部传回来；
- 希望近处证据权重大，越远的证据权重越小。

用 \(\lambda\in[0,1]\) 表示“每向未来多走一步，还保留多少比例”。原本的 reward
折扣仍由 \(\gamma\) 控制，因此从第 \(t\) 步开始，把未来 TD 按距离相加：
...
这不是凭空出现的新目标。它仍然在累加上一节已经得到的预测修正，只是没有让远处修正保持完整权重。

直到这里，再给这条规则命名：
**generalized advantage estimation（广义优势估计）**，简称 **GAE**。

把上式的后半段看成下一时刻的同一种加权和，就得到便于实现的反向递推：
...
GAE 不是步骤正确性的真实标签。它只根据 critic 的前缀预测和最终 reward
构造策略更新权重；
